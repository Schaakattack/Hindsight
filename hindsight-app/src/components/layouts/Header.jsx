...
const Header = ({ isApp, isAction, isLogin, isToken }) => {
    ...
    return (
        <header className={ `relative z-10 header ${ isApp ? 'bg-brand-1100 py-[0.6rem] ' : 'py-5' }` } onBlur={test}>
            <div className={ `container mx-auto flex justify-between items-center ${ isToken ? '' : 'max-w-xl' }`}>
                ...
                <div className="flex gap-5 items-center">
                    ...
                    { !isApp && <DefaultButton className="btn-sm" to='/dapp'>Launch App</DefaultButton> }
                            ...
                            <div className={ `dropdown-menu z-10 absolute w-full min-w-[140px] bottom-[-10px] left-0 ${ actionGroupActive ? 'visible' : 'invisible' }` }>
                                <ul className="dropdown-menu-list rounded-xl bg-brand-1100 absolute  after:gradient-primary after:bg-origin-border after:top-m-1 after:bottom-m-1 after:left-m-1 after:right-m-1 after:border after:border-transparent after:rounded-xl after:z-[-1] after:absolute webkit-mask backdrop-blur-lg">
                                    <li className="rounded-xl hover:bg-header-action-hover-bg-color">
                                        <Link className="dropdown-item flex items-center py-2.5 pl-4 pr-6 gap-2 text-base font-medium tracking-tighter bg-gradient-primary bg-clip-text text-fill-transparent" to='/dapp'>
